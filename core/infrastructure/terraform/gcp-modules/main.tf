...
# Automatically detect project_id from gcloud if not provided via -var or tfvars
locals {
  effective_project_id = var.project_id != "" ? var.project_id : trimspace(chomp(system("gcloud config get-value project")))
}
...
resource "google_storage_bucket" "models" {
  name          = "${local.effective_project_id}-ai-models"
  location      = var.region
  force_destroy = true
          ...
          widget = {
            ...
            xyChart = {
              dataSets = [{
                timeSeriesQuery = {
                  timeSeriesFilter = {
                    filter = "metric.type=\"run.googleapis.com/request_count\" resource.type=\"cloud_run_revision\" resource.label.service_name=\"ai-gateway\""
                  }
                }
              }]
            }

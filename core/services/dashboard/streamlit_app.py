...
MODEL_INFERENCE_URL = "http://34.68.61.142"
# METRICS_API_URL = "http://34.16.20.245"
METRICS_API_URL = os.getenv(
    "ML_CONSUMER_METRICS_URL", "http://136.112.15.237:9090"
).replace("/metrics", "")
...
if page == "🏠 Overview":
    ...
    with col2:
        ...
        with col_b:
            st.link_button(
                "🚪 Gateway", AI_GATEWAY_URL + "/health", use_container_width=True
            )
        with col_c:
            st.link_button(
                "🧠 ML Models", MODEL_INFERENCE_URL, use_container_width=True
            )
    ...
    col_a, col_b, col_c = st.columns(3)
    with col_a:
        st.link_button("🧪 View in MLflow", MLFLOW_URL, use_container_width=True)
    with col_b:
        st.link_button(
            "📖 Model API Docs", f"{MODEL_INFERENCE_URL}/docs", use_container_width=True
        )
    with col_c:
    ...
    # MLflow status
    try:
        mlflow_health = requests.get(f"{MLFLOW_URL}/health", timeout=5)
